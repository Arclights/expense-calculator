...
class CalculationDao(
    ...
    fun getCalculation(year: Int, month: Int): Mono<Calculation> = Flux
        .from(
            dsl.select(
                ...
                .leftJoin(PERSONAL_EXPENSE_CORRECTIONS).on(
                    PERSONAL_EXPENSE_CORRECTIONS.MONTHLY_CALCULATION_ID.eq(
                        MONTHLY_CALCULATIONS.ID
                    )
                )
                ...
                Flux
                    .merge(
                        listOf(
                            ...
                            personalExpensesDao.createUpdatePersonalExpenses(
                                calculationId,
                                c.personalExpenseCorrections
                            )
                        )
                    )
                    .collectList()
            }
            .flatMap { getCalculation(c.year, c.month) }

    private fun createCalculation(c: Calculation): Mono<CalculationId> = Mono
        .fromCallable {
    ...
    private fun updateCalculation(c: Calculation): Mono<CalculationId> = Mono.fromCallable {
...
private fun mapPersonalExpenses(r: List<Record>): List<PersonalExpense> =
    r.distinctBy { it.get(PERSONAL_EXPENSE_CORRECTIONS.ID) }
...
private fun mapCategory(r: Record): Category =
    Category(r.get(CATEGORIES.ID), r.get(CATEGORIES.NAME), r.get(CATEGORIES.COMMENT))